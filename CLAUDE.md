...
All commands accept the `.` dot-prefix (AOL-style). Legacy keyword forms below are fallback aliases.
...
*   **`.subagent <instructions>`**: Spawn a `cavecrew-builder` or `general` sub-agent (choose based on task scope — builder for 1-2 file edits, general for research/multi-step). The sub-agent must first `.meditate` on the relevant code area before executing the task. Returns compressed result. Use for offloading well-scoped work while the main thread continues.
*   **`.cur`** *(alias: `cur`)*: Read the local `cur.md` file (look in the project root, falling back to `~/cur.md`). If absent, report "No cur.md found." If present, output a concise summary: group unfinished items (`- [ ]`) by their nearest `##` header, list each item with its sub-items indented, then state what you believe is the single highest-priority item to work on next (pick the `**blocker**` tag if any, then `## high` items, then `## medium`). Do NOT modify the file. Do NOT add commentary, plans, or fluff. The cur.md is human-edited — keep your output brief and high-level.
*   **`.cur done`** *(alias: `update cur`)*: Re-read `cur.md`. For each unfinished item (`- [ ]`) that you can verify is actually complete: change `[ ]` → `[x]` in-place, then move the entire item block (parent line + all sub-items indented beneath it, regardless of their checkbox state) from its current section down to the `## finished` section at the bottom of the file. Preserve the header hierarchy of the source section but do NOT create new headers in the finished section — just append the items. If a `## finished` section does not exist, create it at the bottom. Also scan any `[x]` items that are NOT yet in `## finished` (stale checkmarks still living in high/medium/etc) and move them down too. After moving, verify no orphaned sub-items remain. If an item was only partially done (some sub-items `[ ]`), do NOT move the parent — leave it in place. Suggest (once, not repeatedly) that the operator add a one-sentence high-level goal/objective to `cur.md` — only add it if the operator explicitly approves. After updating, re-run `.cur` to show the new state.
*   **`.cur add <item> [## section]`** *(alias: `add to cur`)*: Append one or more items to `cur.md`. If no `## <section>` is specified, default to `## high`. Format strictly: parent items as `- [ ] <description>` with 2-space indentation for sub-items (never tabs, never 4 spaces). Place the new item at the END of the target section (before the next `##` header). If the section doesn't exist, create it before `## finished`. Do NOT reorder existing items. Do NOT add commentary. After appending, re-run `.cur` to confirm.
*   **`.fan`**: Adversarial plan validation. Spawn 3 `cavecrew-investigator` sub-agents (monks) in parallel. Each independently meditates on the implementation plan or suggestion the main agent is about to propose. Their sole job is to find flaws, missing edge cases, architectural violations, or simpler alternatives. Each sub-agent returns a compressed caveman report. The main agent MUST read all 3 reports, synthesize the criticism, and either: (a) revise the plan addressing valid criticisms, or (b) explain why each criticism is incorrect. Do NOT proceed with implementation until `.fan` completes or operator overrides. Use before any non-trivial implementation.
...
*   **README Tonality:** Every README must be brief, structured, and plain. No em dashes or en dashes — periods and commas only. No exclamation marks. No hype vocabulary (unleash, supercharge, seamless, game-changing, revolutionize, empower). Short declarative sentences. Truth is the highest virtue — every statement must survive a hostile pedant. Write like explaining to a sharp colleague, never like an ad. Prefer tables and bullet lists over prose paragraphs. If a sentence can be two words shorter, cut it.
