...
*   **`.audit [scope]`**: Scan the project for consistency violations across multiple dimensions in parallel. Spawn one `cavecrew-investigator` per dimension. Dimensions auto-detected from project structure but typically include: (a) UI element consistency — reused components vs duplicated markup, color/token drift, spacing patterns; (b) code organization — file placement conventions, module boundaries, import patterns; (c) naming — symbol naming conventions, file naming, route naming; (d) deployment/config — environment variable patterns, build tooling, CI consistency. Each sub-agent returns a compressed report of violations found. The main agent synthesizes into a single prioritized list. If no scope is given, audit all dimensions. If the project has no UI (e.g. a library, CLI tool), skip UI dimension.
