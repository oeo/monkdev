...
*   **`.check [N]`**: Adversarial plan validation. Spawn N `cavecrew-investigator` sub-agents (monks) in parallel (default 5). Each independently meditates on the implementation plan or suggestion the main agent is about to propose. Their sole job is to find flaws, missing edge cases, architectural violations, or simpler alternatives. Each sub-agent returns a compressed caveman report. The main agent MUST read all reports, synthesize the criticism, and either: (a) revise the plan addressing valid criticisms, or (b) explain why each criticism is incorrect. Do NOT proceed with implementation until `.check` completes or operator overrides. Use before any non-trivial implementation.
