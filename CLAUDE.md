...
*   **`#canon [path]`**: Find facts written down in more than one language and name the one place each should live. Run `monk_canon <path>` first: it clusters identifiers, literals, numbers and declaration names appearing in two or more languages and prints file:line for every copy. It is a candidate list, not a verdict, and roughly half of a root-level run is vocabulary rather than fact. Read the copies with `monk_catfiles` before calling anything drift.
