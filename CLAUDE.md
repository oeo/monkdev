...
4. **Ingest (The Artifact Protocol):** Use `monk_context <directory>` to ingest entire modules cohesively as structured XML. **CRITICAL:** Modern context windows are massive. Do NOT write to a temporary file or use offsets unless the token load exceeds 500,000 tokens AND the user hasn't explicitly authorized a massive ingestion. If the user commands you to `meditate deeply` and approves the token load, simply run `monk_context <directory>` directly via MCP to stream the entire codebase into your active memory instantly. Only fallback to the `--out` file and surgical `monk_catfiles` reading if the token load is astronomical (>1,000,000 tokens) or if you only need targeted understanding.
